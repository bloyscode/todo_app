...
//           <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Task Mate</h1>
//           <p className="text-green-200 md:text-gray-400 text-sm mt-1">Your friendly task companiom</p>
//         </div>
...
export default function App() {
  ...
  const updateTaskTitle = (id: number, newTitle: string) => {
    ...
    axios.put(`http://localhost:8081/tasks/${id}`, { ...task, title: newTitle })
      .catch((err) => {
        ...
        setTasks(tasks.map((t) => (t.id === id ? { ...t, title: originalTitle } : t)));
      });
  };
  ...
  const updateTaskDate = (id: number, newDate: string) => {
    ...
    axios.put(`http://localhost:8081/tasks/${id}`, { ...task, date: newDate })
      .catch((err) => {
        ...
        setTasks(tasks.map((t) => (t.id === id ? { ...t, date: originalDate } : t)));
      });
  };
  ...
  const TaskItem = ({ task, toggleComplete, deleteTask, updateTaskTitle, updateTaskDate }: {
    ...
    // Sync local editing state with task prop when editing is closed
    useEffect(() => {
      if (!isEditing) {
        setNewTitle(task.title);
        setNewDate(task.date);
      }
    }, [task.title, task.date, isEditing]);
    ...
    return (
      <li
        ...
        className={`group flex justify-between items-center p-4 md:p-5 bg-gray-800 border border-gray-700 rounded-2xl shadow-lg transition-all duration-300 ${task.completed ? "opacity-70" : "opacity-100"}`}
      >
        {/* Removed Checkbox input div here */}
        <div className="flex items-center gap-4 md:gap-5 min-w-0 flex-grow">
          ...
          <div className="flex flex-col min-w-0 flex-grow">
            {isEditing ? (
              ...
              <span
                className={`text-lg font-medium transition-all cursor-pointer truncate ${task.completed ? "text-gray-500" : "text-white"}`}
                onClick={() => !task.completed && setIsEditing(true)}
                title={task.title}
              >
                {task.title}
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          {!task.completed && (
            <div className="flex items-center gap-2">
              ...
              {/* Complete Button (Only visible when not editing) */}
              {!isEditing && (
                <button
    ...
    <div className="h-screen w-full bg-gray-900 flex flex-col md:flex-row font-sans overflow-hidden text-white">
      ...
      <aside className="w-full md:w-96 flex-shrink-0 bg-gray-800 shadow-xl z-10 flex flex-col">
        ...
        <div className="px-6 md:px-10 flex-1 flex flex-row md:flex-col gap-2 md:gap-4 py-4 md:py-8 overflow-x-auto md:overflow-visible items-center md:items-stretch no-scrollbar">
          ...
          {/* Added a button for Completed tasks visualization */}
          <button
            onClick={() => setActiveTab('pending')} // Keeps the current logic for displaying completed tasks under pending tab but allows for a visual counter
            className="px-5 py-3 rounded-full md:rounded-xl font-medium transition-all duration-200 text-left flex items-center justify-between whitespace-nowrap text-gray-400 hover:bg-gray-700 hover:text-white"
          >
            <span>Total Completed</span>
            <span className="text-xs px-2 py-0.5 rounded-full min-w-[24px] text-center font-semibold bg-gray-700 text-green-400">
